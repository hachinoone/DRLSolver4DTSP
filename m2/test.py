...
class DistanceMatrix:
    ...
    def __getddd__(self, st, a, b, t):
        ...
        z2 = z * z
        z3 = z2 * z
        #print(a0.size(), a1.size(), a2.size(), a3.size(), z.size(), z2.size(), z3.size())
        res = a0 + a1 * z + a2 * z2 + a3 * z3
        minres = (a0 + b0) * 0.05
        maxres = (a0 + b0) * 5
        res,_ = torch.max(torch.cat((res.unsqueeze(-1), minres.unsqueeze(-1)), dim = -1), dim = -1)
...
def run(opts):
    ...
    ci = Cities()
    mat = DistanceMatrix(ci, load_dir='m2/data.csv', max_time_step = 12)
    ...
    model1 = np.loadtxt('m2/emb.txt', delimiter=' ')
    ...
    checkpoint2 = torch.load('m2/T2V_normal_19.pt')
    ...
    val_dataset = TSPDataset(ci, size=opts.graph_size, num_samples=opts.val_size, filename='data_nodes/node_19.txt', filename1='data_nodes/add_data_19_0.txt',filename2='data_nodes/bdd_data_19_0.txt', distribution=opts.data_distribution)
    ...
    modelz = baseline.baseline.model
    ans, cost = roll(mat, modelz, val_dataset, opts, model1, model2)
